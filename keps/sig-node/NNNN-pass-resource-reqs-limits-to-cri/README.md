# KEP-NNNN: Pass Container Resource Requests and Limits to CRI
...
platform-optimized container runtimes,
...
As a VM-based container runtime developer, I want to allocate/expose
enough RAM, hugepages, GPU memory, Intel SGX EPC memory sections and
other resources for the VM to ensure that all containers in the pod
are guaranteed to get the resources they require.

#### Story 2

As a platform-optimized CRI runtime developer, I want to know detailed
container resource requests of all resources handled by kubelet to be
able to make optimal, platform specific, resource allocations. Some of
the resources may be handled outside cgroups and be container runtime
specific details.
...
We propose that kubelet discloses full `resources` information to CRIs
in *new* `ResourceConfig` field (of type `PodResourceConfig`). The
field is added to *existing* `PodSandboxConfig`, `ContainerConfig` and
`UpdateContainerResourcesRequest` structures. It contains the
information of resource requests of all containers in a pod.
...
requested CPU, memory and `intel.com/qat` resources are reserved to
containers so that accesses between hardware resources are as
efficient as possible on the underlying hardware.
